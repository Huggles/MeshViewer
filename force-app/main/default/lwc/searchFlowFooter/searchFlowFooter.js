...
export default class SearchFlowFooter extends LightningElement {
    ...
    connectedCallback() {
        // the theory is that components fire a registration event AFTER the listener has been registered.
        // For that the firing needs to be done from the renderedCallback method
        ...
        fireEvent(null, 'componentRegistrationOpen', { object : this, pageRef : this.pageRef});
    }
    ...
    handleComponentRegistration(event) {
        if (event.component && event.pageRef == this.pageRef){
            this.registeredComponents.add(event.component);
        }
    }

    handleComponentValidationDone(event) {
        if(event.pageRef == this.pageRef){
            if (this.availableActions.find(action => action === 'NEXT') ||
                this.availableActions.find(action => action === 'FINISH')) { // should be true but just to be certain,
    ...
    handleNextClick(event) {
        ...
        let payload = { object : this, pageRef : this.pageRef};
        fireEvent(null, 'validationRequest', payload);
    }
